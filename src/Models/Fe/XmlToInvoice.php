...
use Areaseb\Core\Models\{Company, Invoice, Item, Exemption, Media};
use Carbon\Carbon;

class XmlToInvoice extends Primitive
{
    ...
    private function getCf($xml)
    {
        if(isset($xml->FatturaElettronicaHeader->CessionarioCommittente->DatiAnagrafici))
        {
            return $xml->FatturaElettronicaHeader->CessionarioCommittente->DatiAnagrafici->CodiceFiscale;
        }
        return $xml->FatturaElettronicaHeader->CessionarioCommittente->IdFiscaleIVA->IdCodice;
    }
    ...
    private function companyExists($xml)
    {
        ...
        try
        {
            if(isset($xml->FatturaElettronicaHeader->CessionarioCommittente->DatiAnagrafici))
            {
                ...
                if(isset($xml->FatturaElettronicaHeader->CessionarioCommittente->DatiAnagrafici->CodiceFiscale))
                {
