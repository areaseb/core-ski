    ...
    @if(count(request()->input()))
        <div class="form-group">
            <ul>
                ...
                @endif
                @if(request()->get('tipo'))
                    <li>Tipo: {{Areaseb\Core\Models\Client::find(request()->get('tipo'))->nome}} <i class="fas fa-check-circle text-success"></i></li>
                @endif
                ...
                @if(request()->get('origin'))
                    <li>Origin: {{request()->get('origin')}} <i class="fas fa-check-circle text-success"></i></li>
                @endif
                @if(request()->get('sector'))
                    <li>Settore: {{request()->get('origin')}} <i class="fas fa-check-circle text-success"></i></li>
                @endif
            </ul>
            ...
            <input type="hidden" class="form-control" name="sector" value="{{request()->get('sector')}}">
            <input type="hidden" class="form-control" name="origin" value="{{request()->get('origin')}}">
        </div>
    @else
        tutti i contatti
    @endif
