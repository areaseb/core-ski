...
@include('areaseb::layouts.elements.title', ['title' => $list->nome.' ('.$list->count_contacts.')'])
                ...
                <div class="card-body">
                    @can('lists.write')
                        <div class="btn-group mb-1" style="width:100%;">
                            ...
                            {!! Form::select('contact_id',[''=>'']+$availables, null) !!}
                        </div>
                        ...
                        <tbody>
                            @foreach($contacts as $contact)
                                <tr id="row-{{$contact->id}}">
                                    ...
                                    <td>{{$contact->tipo}}</td>
                                    <td>
                                        @include('areaseb::core.components.contact-stats')
                                    </td>
...
$('select[name="contact_id"]').select2({width:'100%', placeholder: 'Aggiungi contatto'});
$('select[name="contact_id"]').on('change', function(){
    ...
    axios.post(baseURL+'add-contact-to-list', data).then(response => {
