                    ...
                    @foreach($company->contacts as $contact)
	                    ...
	                    <div class="table-responsive">
	                    	<table class="table table-sm table-bordered table-striped table-php">
	                            ...
	                            <tbody>
	                                @foreach(\Areaseb\Core\Http\Controllers\ContactController::getCollettiviApertiCliente($contact->id) as $dd)
		                                <tr>	                                	
											<td> <a href="/collective/{{$dd->collett_id}}">{{$dd->collettivo}}</a></td>
											<td>
												@foreach($dd->date->where('partecipante', $contact->id)->orderBy('giorno')->pluck('giorno')->toArray() as $data)
